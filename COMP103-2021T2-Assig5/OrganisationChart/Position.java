...
public class Position {
    ...
    public void addToTeam(Position newMemb){
        if (newMemb == null) return; // if newMemb is non existent then return
        else{
            newMemb.manager= this; // otherwise the manager for newmemb is this position
            team.add(newMemb); // and now add newmemb to the team
        }
    ...
    public void removeFromTeam(Position teamMemb){
        if(teamMemb == null) return; // check if teammemb is non existent
        else{
            this.team.remove(teamMemb); // remove from this team
            teamMemb.manager=null; // and now i removed its manager
        }
