...
public class OrganisationChart {
    ...
    private void drawTree(Position pos) {
        ...
        for(Position node : pos.getTeam()){ // for each child of pos draw them onto the tree
            drawTree(node);
        }
    ...
    private Position findPosition(double x, double y, Position pos){
        if(pos.on(x,y)){ // if pos is on x,y then return pos
            return pos;
        }
        else{ // otherwise for each node in the pos.getTeam() create a temporary position and set it as the node on x y ( if there is one)  and if after the for each loop is over and there is a node on the x y then return it
            for(Position node : pos.getTeam()){
               Position temp = this.findPosition(x,y,node);
               if(temp != null) {
                   return temp;
               }
            }
    ...
    public void addNewPosition(Position newPos, Position target){
        if(newPos == null || target == null)return; // making sure the arguments are valid otherwise return
        else{
            target.addToTeam(newPos); // target add to the team the new pos
        }
    ...
    private void movePosition(Position pos, Position target) {
        if ((pos == null) || (target == null)){return;}   // check the arguments
        if(pos != organisation && inSubtree(pos,target) == false){ // pos can not be the root node and pos can not be moved to a target in its own sub tree
            Position manager = pos.getManager(); // find the manager
            manager.removeFromTeam(pos); // remove from the team
            target.addToTeam(pos); // now add it to the new team
        }
    ...
    public void removePosition(Position pos){
        if(pos == null) return; // check if pos is not null
        if(pos.isManager() == false){ // if the pos is not a manger then we find its manager and use that to remove our pos
    ...
    private boolean inSubtree(Position pos, Position subtree) {
        if(pos == null || subtree == null)return false; //  check if arguments are valid
        if(pos == subtree)return true; // if pos is equal to the subtree then return true
        else{
            for(Position posi : pos.getTeam()){
                boolean check = inSubtree(posi,subtree); // recursive call to check every position in the team to make sure it is not a subtree of any
