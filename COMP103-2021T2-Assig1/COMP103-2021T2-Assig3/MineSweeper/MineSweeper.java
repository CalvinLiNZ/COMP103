...
public class MineSweeper {
     ...
     */
    public void tryExpose(int row, int col){ // tests to see if the square selected is a bomb or not and if it is it will end the game.
     ...
     */
    public void exposeSquareAt(int row, int col){ // exposes the square where clicked and if adjacent squares are 0 then expose the squares
        Square square = squares[row][col];
        if (square.isExposed()){ //if square is already exposed then do nothing
            return;
        } else{// expose square and its surrounding squares
            square.setExposed();
            square.draw(row,col);
            if(square.getAdjacentMines()==0){
                for(int i = row-1; i< row+2; i++){
                    for( int z = col-1; z < col+2 ; z++){
                        if( i>= 0 && z>= 0 && i < ROWS && z<COLS){
                            exposeSquareAt(i,z);
                        }
                    }
                }
    ...
    public boolean hasWon(){
        for (int i = 0; i<ROWS; i++){
            for(int y = 0; y<COLS; y++){
                ...
                if ((square.hasMine() && square.isExposed()) || (!square.hasMine() && !square.isExposed())){
                    return false;
                }
            }
        }
