...
public class HospitalERCore {
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            for(Patient patients : treatmentRoom){
                placeHolder.add(patients);
            }
            for(Patient patient : placeHolder) {
                if (patient.completedCurrentTreatment()) {
                    ...
                    if (patient.getPriority() == 1) {
                        patientNumPri++;
                        timesPri += patient.getWaitingTime();
                    }
                    int time = patient.getWaitingTime();
                    times += time;
                } else {
                    patient.advanceTreatmentByTick();
                }
            }
            Patient nextPatient = new Patient(2,1);
            for(Patient pat : waitingRoom){
                pat.waitForATick();
                for(Patient p : waitingRoom){
                    if(p.compareTo(pat) == 1){
                        nextPatient = p;
                    } else {
                        nextPatient = pat;
                    }
